...
svg.spline <- function(count_in, locus_in, x_in = NULL, df = 5, block_size = 1000){
  ...
  ## Center spline basis
  Z <- sweep( Z, 2, colMeans(Z), "-", check.margin = FALSE )
  ...
  for(k in seq_along(starts))
  {
    ...
    ## Free memory
    rm(block, XtY, beta, SSR, sy, sy2, SST)
